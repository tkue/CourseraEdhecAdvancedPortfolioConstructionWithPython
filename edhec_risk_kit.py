...
def skewness(r):
    ...
    sigma_r = r.std(ddof=0)  # degree of freedom (so we don't do 1-r)
...
def is_normal(r, level=0.01):
    ...
    if isinstance(r, pd.DataFrame):
        return r.aggregate(is_normal)  # aggregate() applies to every column
    else:
...
def cvar_historic(r, level=5):
    ...
    """
    if isinstance(r, pd.Series):
        is_beyond = r <= -var_historic(r, level=level)  # negative because we had negative in the var_historic() method
